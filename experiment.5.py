...
def create_people_3(n):
    ...
    for i in range(n):
        ...
        person_view_offer_sensitivity = Categorical(['background', 'offer_age', 'web', 'email', 'mobile', 'social'],
                                                    [g_inv(0.20) - 2, -abs(g_inv(0.01) / float(1 * 24 * 30)), 0, 0.5, 0.5,
                                                     1])
        person_make_purchase_sensitivity = Categorical(
            ['background', 'time_since_last_transaction', 'last_viewed_offer_strength', 'viewed_active_offer'],
...
def main(args):
    ...
    people = list()
    people.extend(create_people_0(2200))
    people.extend(create_people_1(2100))
    people.extend(create_people_2(2100))
    people.extend(create_people_3(1300))
    people.extend(create_people_4(1200))
    people.extend(create_people_5(1100))
